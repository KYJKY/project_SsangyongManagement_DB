...
또, PL/SQL화 하여 긴 쿼리문이 아닌, 짧은 쿼리문에 매개변수를 입력하여 하나의 메소드처럼 관리할 수 있게 구현해보았는데 이 부분이 상당히 흥미로웠다.
팀장 규준님이 Oracle Cloud를 사용하자는 제안을 통해 Oracle Cloud를 통해 하나의 DB에서 트랜젝션에 유의하면서 개발해볼 수 있어서 좋았다.

### \- 어려웠던 점 -
DB를 설계하면서 내 생각보다 고려해야할 부분이 많았다는 점이 어려웠다. 계속 똑같은 요구사항만 보니까 지나친 당연한 부분을 캐치못하는 경우가 많았다.
PL/SQL을 작성하는데 생산성이 낮았다. 내가 익숙치 않아서 그랬을 수도 있지만, 하나의 구현을 위해 저장 프로시저를 두 번 정의하는게 과연 효율적인지에 대한 의문이 남았다.
